...
额外引用实验 Redis 模板。缓存故障内容已经并入多级缓存课程，不创建额外实验资源。
...
Go 适配器只调用创建、重置和销毁三个受限存储过程。
实验数据库和数据库账号不建立独立登记表，
后续由 `lab_resources` 作为资源事实源。
...
动态网络、数据库和数据库账号。
Docker 清理目标必须带匹配的实验标签。
数据库清理目标必须由已校验的 `labId` 确定性生成，
不得按名称前缀扫描并删除数据库。

`RECONCILE_RESOURCES` 从以下来源汇总状态：

* 调用方在 `expectedLabIds` 中提供的期望实验集合；
* Docker 管理标签；
* Nginx 片段文件。
