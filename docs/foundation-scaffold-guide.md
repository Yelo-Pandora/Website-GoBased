...
MySQL 额外映射到 `127.0.0.1:3306`，仅用于宿主机本地受信任数据库管理工具。
该映射在 MVP 发布后继续保留，但不得改为监听全部网卡。
...
- Edge Nginx 是唯一公网入口；MySQL 永久保留仅绑定回环地址的本机管理端口。
...
- 环境变量是否有明确消费者。
- 是否只有 Edge Nginx 使用公网端口，MySQL `ports` 是否仍绑定 `127.0.0.1`。
- 服务是否只加入必要网络。
...
- 平台账户和编排器账户符合最小权限要求。
- Edge Nginx 是唯一公网入口，MySQL 只能通过宿主机回环地址访问。
