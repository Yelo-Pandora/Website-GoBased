...
首页是普通用户课程页面，左侧课程目录同时包含理论课程和实验课程。
普通用户入口不显示 Swagger UI 或 OpenAPI 文件；本机运维入口为
`http://127.0.0.1:8081`。
...
1. 登录并确认出现课程列表。
2. 选择“应用集群与负载均衡”。
3. 确认默认显示课程正文，再切换到“实验”页签。
4. 点击“创建实验”。
5. 观察状态从“准备中”进入“运行中”。
6. 确认拓扑出现实验网关、`app-1` 和实验数据库。
7. 确认空闲时限和最长时限开始倒计时。
8. 切到“课程”页签阅读正文，再切回“实验”并确认快照立即刷新。
9. 刷新浏览器，确认登录态、实验 ID、拓扑、最新操作和“实验”页签能够恢复。
10. 点击“重置”，确认最新操作变为“重置实验 / succeeded”，实验重新回到“运行中”。
11. 点击“结束”并确认，等待状态变为“已结束”，结束原因为“用户主动结束”。
...
E2E 覆盖登录、纯理论正文、课程与实验切换、活动实验恢复、创建至 `Running`、拓扑、
倒计时、重置、主动结束以及 390px 移动视口横向溢出检查。
设置 `RUN_SHORT_LIFECYCLE=1` 并配合短时生命周期配置时，还会验证 `Expiring` 和自动空闲回收。
