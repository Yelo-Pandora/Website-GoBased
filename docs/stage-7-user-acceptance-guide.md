...
处理量、丢弃量和负载状态，前端只根据同步结果播放动画。

## 标准流程

1. 登录测试账号并创建“应用集群与负载均衡”实验。
2. 在“真实请求流”发送一批 60 个教学等效订单。
3. 确认小球经过实验网关并进入响应指定的实例。
4. 增加 `app-2`，等待最新操作变为 `succeeded`。
5. 将固定权重调整为 `app-1 = 20`、`app-2 = 80`。
6. 将 `app-2` 性能调整为 30%，确认 CPU 与有效容量同步下降。
7. 连续发送批次，观察 `processed`、`partially_processed` 或 `dropped`，并确认数量守恒。
8. 删除一个实例，确认集群仍保留至少一台应用。
9. 结束实验并确认受管容器、网络、Nginx 片段和实验数据库被清理。
