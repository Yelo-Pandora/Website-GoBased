...
阶段三建立实验控制面的数据一致性和后台执行基础，但不创建 Docker 实验资源。
公开端点 `POST /api/v1/labs` 继续完成 Session 与 CSRF 校验，随后返回
`501 LABS_NOT_IMPLEMENTED`。阶段四完成受限资源编排、阶段五打通首个实验闭环后，才会把
该端点切换为 `202 Accepted`。
...
本阶段不实现 Docker、Nginx、实验数据库存储过程适配器、资源补偿、实验快照和 SSE。

## 3. 数据库变化

迁移 `002_lab_control_queue.sql` 创建 `lab_control_locks`，并写入唯一的
`global-admission` 行。
...
3. 锁定 `global-admission`，串行计算全局活动实验和临时容器预留量。
4. 校验课程是否属于后端允许的实验场景。
5. 检查单活动实验和资源配额。
6. 同时写入 `lab_sessions`、`lab_operations` 和 `course_progress.last_lab_id`。
7. 提交事务后，后台 Worker 才能领取该操作。
